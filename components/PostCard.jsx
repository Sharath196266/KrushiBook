  ...
  return (
      ...
      <View style={!isActive("/profile")? ((item?.file ? (styles.content):(isContentNoFilePLat))) : (item?.file ? (styles.content):(isContentNoFile))}>
        <View style={styles.postBody}>
          <Text style={Platform.OS!=="web"?([{margin:5,fontWeight:theme.fonts.text}]):(item?.file ? ([{margin:5,fontWeight:theme.fonts.text}]):([{margin:5,fontWeight:theme.fonts.textDark,fontSize:hp(3)}]))}>{item?.body}</Text>
          {item?.file && item?.file.includes('postImages') && (
            
            <Image source={getSupabaseFileUrl(item?.file)} transition={100} 
            style={
              !isActive('/profile')?
              (Platform.OS !== 'web'?[styles.postMedia]:[styles.postMediaWeb,{height:hp(60)}]
